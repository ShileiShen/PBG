...
def arc_minus_x_up_draw(d_angle, total_length, width):
    ...
    for n in range(round(numpy.pi / d_angle)):
        if (position.length < total_length) and (position.angle >= numpy.pi / 2):
            createArcRes(width, res_R, position.angle,
                         position.angle - d_angle)
            position.length += d_angle * R
            position.add_angle(-d_angle)
        else:
            break
...
# draw a structure
position = Position.Position(
    x=chip_width / 2 - l_res / 2, y=chip_length / 2 - edge_offset, direction='-x')
createPoly(width=t_Zhigh, length=10, direction='-x',
           final_width=t_res)  # tapering at the end
