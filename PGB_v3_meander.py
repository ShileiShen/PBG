...
def createPoly(position, x, y):
	poly = gdspy.Rectangle((position.x, position.y),
	                       (position.move_x(x), position.move_y(y)))
	return poly
...
def ZlowBack(position, l_Zhigh, l_Zlow, t_Zhigh, t_Zlow):
	...
	Zlow = gdspy.Rectangle((position.x, position.y),
	                       (position.x + l_Zlow, position.y + t_Zlow))
	return Zlow
...
def bendRightZhighRes(position, cell, l_Zhigh, l_Zlow, t_Zhigh, t_Zlow, R_inner):
	...
	Zlow2 = gdspy.Rectangle((position.x, position.y),
	                        (position.x + l1, position.y + t_Zhigh))
	cell.add(Zlow2)
	...
	Zlow2 = gdspy.Rectangle((position.x, position.y),
	                        (position.x + t_Zhigh, position.y - l_residual))
	cell.add(Zlow2)
	...
	Zlow4 = gdspy.Rectangle((position.x, position.y),
	                        (position.x - l2, position.y + t_Zhigh))
	cell.add(Zlow4)
...
def bendRightZlowEND(position, cell, l_Zhigh, l_Zlow, t_Zhigh, t_Zlow, R_inner):
	...
	Zlow2 = gdspy.Rectangle((position.x, position.y),
	                        (position.x + l1, position.y + t_Zlow))
	cell.add(Zlow2)
...
def resonator(position, cell, l_Zhigh, l_Zlow, t_Zhigh, t_Zlow):
	...
	# upper y coordinate of the resonator
	y_init = position.y + 0.5 * (t_Zlow - t_res) + t_res
	...
	gndRes_bottom = gdspy.Rectangle(
			(position.x, position.y - gap), (position.x + l_res, position.y - gap + gndRes_h))
...
cell.add(Zlow(cursor, l_Zhigh, l_Zlow, t_Zhigh, t_Zlow))
cell = bendRightZhigh(cursor, cell, l_Zhigh, l_Zlow,
                      t_Zhigh, t_Zlow, R_inner_high)

cell = bendLeftZlow(cursor, cell, l_Zhigh, l_Zlow,
                    t_Zhigh, t_Zlow, R_inner_low)
cell = bendRightZhigh(cursor, cell, l_Zhigh, l_Zlow,
                      t_Zhigh, t_Zlow, R_inner_high)
...
cell = bendLeftZlow(cursor, cell, l_Zhigh, l_Zlow,
                    t_Zhigh, t_Zlow, R_inner_low)
cell = bendRightZhighRes(cursor, cell, l_Zhigh, l_Zlow,
                         t_Zhigh, t_Zlow, R_inner_high)

cell = resonator(cursor, cell, l_Zhigh, l_Zlow, t_Zhigh, t_Zlow)

cell = bendLeftZhighRes(cursor, cell, l_Zhigh, l_Zlow,
                        t_Zhigh, t_Zlow, R_inner_high)
cell = bendRightZlow2(cursor, cell, l_Zhigh, l_Zlow,
                      t_Zhigh, t_Zlow, R_inner_low)

cell = bendLeftZhigh2(cursor, cell, l_Zhigh, l_Zlow,
                      t_Zhigh, t_Zlow, R_inner_high)
