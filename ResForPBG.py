...
for i in range(number):
    ...
    Zlow1 = gdspy.Rectangle((0+k*(l_Zlow+l_Zhigh),0),(l_Zlow + k*(l_Zlow+l_Zhigh),t_Zlow))
    
    Zhigh1= gdspy.Rectangle((l_Zlow + k*(l_Zlow+l_Zhigh) , (t_Zlow-t_Zhigh)/2.),(l_Zlow + l_Zhigh + k*(l_Zlow+l_Zhigh),(t_Zlow-t_Zhigh)/2. + t_Zhigh))
#    cell.add(Zlow1)
#    cell.add(Zhigh1)
    k=k+1

res=gdspy.Rectangle( ( k*(l_Zlow+l_Zhigh), (t_Zlow-t_res)/2.), ( k*(l_Zlow+l_Zhigh) + l_res ,(t_Zlow-t_res)/2. + t_res ) )
cell.add(res)


c_left=gdspy.Rectangle( ( k*(l_Zlow+l_Zhigh)-c_gap-c_length, (t_Zlow-t_res)/2. ), (k*(l_Zlow+l_Zhigh)-c_gap , (t_Zlow-t_res)/2. + t_res)  )
cell.add(c_left)

c_right=gdspy.Rectangle( ( k*(l_Zlow+l_Zhigh) + l_res+c_gap ,(t_Zlow-t_res)/2.), ( k*(l_Zlow+l_Zhigh) + l_res+c_gap+c_length ,(t_Zlow-t_res)/2. + t_res))
cell.add(c_right)
...
for i in range(number):
    ...
    Zlow2 = gdspy.Rectangle((offset_X1+l_Zhigh+k*(l_Zlow+l_Zhigh),0),(offset_X1+l_Zlow+ l_Zhigh+ k*(l_Zlow+l_Zhigh),t_Zlow))
    
    Zhigh2= gdspy.Rectangle((offset_X1+ k*(l_Zlow+l_Zhigh) , (t_Zlow-t_Zhigh)/2.),(offset_X1+ l_Zhigh + k*(l_Zlow+l_Zhigh),(t_Zlow-t_Zhigh)/2. + t_Zhigh))
#    cell.add(Zlow2)
#    cell.add(Zhigh2)
    k=k+1
...
gnd_upper=gdspy.Rectangle( (k*(l_Zlow+l_Zhigh) ,t_Zlow+gap), (k*(l_Zlow+l_Zhigh) + l_res  , t_Zlow+gap+w_gnd) )
gnd_bottom=gdspy.Rectangle( (k*(l_Zlow+l_Zhigh) ,-gap), (k*(l_Zlow+l_Zhigh) + l_res  ,-gap-w_gnd) )

#cell.add(gnd_bottom)
#cell.add(gnd_upper)

print( "total length of the structure: %2.1f and width: %2.1f" % (offset_X1+offset_X2, 2*w_gnd+2*gap+t_Zlow))
...
gndRes_upper=gdspy.Rectangle((x_start- c_gap - c_length,t_Zlow+gap ), (x_start+l_res-2*delta_x+w_c +c_gap +c_length, t_Zlow+gap-gndRes_h))
gndRes_bottom=gdspy.Rectangle((x_start- c_gap - c_length,-gap ), (x_start+l_res-2*delta_x+w_c+c_gap +c_length, -gap+gndRes_h))
...
pointsLowRight=[(x_start+l_res-2*delta_x+w_c, -gap), (x_start+l_res-2*delta_x+w_c, -gap+gndRes_h), (x_start+l_res-2*delta_x+delta_x, -gap)]               
pointsTopLeft=[(x_init, t_Zlow+gap), (x_start, t_Zlow+gap), (x_start,t_Zlow+gap-gndRes_h)]           
pointsTopRight=[(x_start+l_res-2*delta_x+w_c, t_Zlow+gap), (x_start+l_res-2*delta_x+delta_x,t_Zlow+gap), (x_start+l_res-2*delta_x+w_c,t_Zlow+gap-gndRes_h)]
